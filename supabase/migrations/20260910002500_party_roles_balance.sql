-- v0.12.6 role balance: mage should specialize in armor penetration, not dominate basic DPS.
update public.game_skill_defs
set damage_multiplier=1.55,
    defense_ignore=0.50,
    description='3번째 공격마다 방어를 절반 무시하는 마력탄을 발사합니다.'
where id='arcane_bolt';

update public.game_candidates
set hp_base=round(86*(case growth_grade when 'S' then 1.16 when 'A' then 1.10 when 'B' then 1.05 else 1 end)),
    atk_base=round(22*(case growth_grade when 'S' then 1.16 when 'A' then 1.10 when 'B' then 1.05 else 1 end)),
    def_base=round(5*(case growth_grade when 'S' then 1.16 when 'A' then 1.10 when 'B' then 1.05 else 1 end)),
    spd_base=round(11*(case growth_grade when 'S' then 1.16 when 'A' then 1.10 when 'B' then 1.05 else 1 end)),
    crit_base=0.06,
    evade_base=0.04,
    power_base=54
where family='imp';
