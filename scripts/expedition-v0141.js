...
(()=>{
  ...
  const hpFor=(e,m)=>{const bs=e?.battle_state||{},max=Math.max(1,Number(bs.partyMaxHp?.[m.id]||monsterStats(m).hp)),hp=Math.max(0,Math.min(max,Number(bs.partyHp?.[m.id]??max)));return {hp,max,pct:hp/max*100}};
  function slots(ids,e,locked=false){return Array.from({length:4},(_,i)=>{const m=monster(ids[i]);if(!m)return `<div class="exp-party-slot empty"><span>+</span><small>${locked?'재편 시 배치':'빈 자리'}</small></div>`;const h=hpFor(e,m);return `<div class="exp-party-slot ${h.hp===0?'down':''}" data-exp-monster="${esc(m.id)}"><span class="exp-party-avatar">${charSvg(monsterAsset(m.family),'party-sprite '+formClass(m))}</span><b>${esc(formName(m))}</b><small data-exp-hp>${h.hp===0?'전투불능':`HP ${fmt(h.hp)} / ${fmt(h.max)}`}</small><i><em style="width:${h.pct}%"></em></i></div>`}).join('')}
  ...
  function refresh(){
    ...
    sheet.querySelectorAll('[data-exp-monster]').forEach(slot=>{const m=monster(slot.dataset.expMonster);if(!m)return;const h=hpFor(e,m),label=slot.querySelector('[data-exp-hp]'),bar=slot.querySelector('i>em'),text=h.hp===0?'전투불능':`HP ${fmt(h.hp)} / ${fmt(h.max)}`;if(label&&label.textContent!==text)label.textContent=text;if(bar)bar.style.width=`${h.pct}%`;slot.classList.toggle('down',h.hp===0)});
