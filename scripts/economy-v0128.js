...
(()=>{
  ...
  function workshop(){
    ...
    modal(`<div class="sheet-head"><div><div class="section-kicker">WORKSHOP</div><h2>제작소</h2><p>제작이 끝나면 완료 버튼을 눌러 창고로 수령합니다.</p></div><button class="close-btn" data-action="close">×</button></div><div class="facility-upgrade"><span><small>제작소 Lv.${lv}</small><b>작업 ${active.length}/${cap}${ready.length?` · 완료 대기 ${ready.length}`:''}</b></span><button class="soft-btn" data-action="upgrade" data-id="workshop">확장 ${fmt(facilityCost('workshop'))}G</button></div><div class="economy-note">완료된 제작품은 자동으로 창고에 들어오지 않습니다. <b>완료</b>를 눌러 수령하세요. 완료 대기 중이어도 다음 대기열은 계속 진행됩니다.</div>${jobRows('craft')}<div class="recipe-list">${[...(S.recipes||[])].sort((a,b)=>Number(b.workshop_level<=lv&&craftMaxQty(b)>0)-Number(a.workshop_level<=lv&&craftMaxQty(a)>0)||a.workshop_level-b.workshop_level||a.sort_order-b.sort_order).map(r=>{const inputs=Object.entries(r.inputs||{}),max=craftMaxQty(r),locked=Number(r.workshop_level||1)>lv,disabled=locked||max<1||queueFull;return `<div class="recipe-row"><div class="recipe-icon">${itemSvg(itemAsset(r.output_item))}</div><div class="recipe-main"><b>${esc(r.output_item)}</b>${locked?`<p class="campaign-recipe-lock">제작소 Lv.${r.workshop_level} 필요</p>`:''}<div class="recipe-meta"><span>판매 <strong>${fmt(r.sale_gold)}G</strong></span><span>1개 제작 ${durationText(r.craft_seconds)}</span><span>1개 판매 ${durationText(Math.ceil(Number(r.craft_seconds||1)/1.2))}</span></div><div class="recipe-inputs">${inputs.map(([id,q])=>`<span class="${inventoryQty(id)<Number(q)?'short':''}">${esc(id)} ${fmt(inventoryQty(id))} / 개당 ${fmt(q)}</span>`).join('')}</div></div><button class="primary-btn" data-econ-action="craft-setup" data-id="${esc(r.id)}" ${disabled?'disabled':''}>${locked?'시설 조건':queueFull?'대기열':'제작'}</button></div>`}).join('')}</div>`,'economy-sheet economy-v0128-sheet');
    updateTimers();
  }
  ...
  async function claim(type,id){
    ...
    try{
      ...
      if(msg.includes('storage_full')){warehouseModal();toast('창고를 확장하거나 재고를 판매한 뒤 다시 수령하세요. 완성품은 보관됩니다.');}
      else if(msg.includes('job_not_ready'))toast('아직 작업이 완료되지 않았습니다.');
