...
(()=>{
  ...
  function progressText(s){return unlocked(s)?'보스 출현 해금 · 1.5% / 최대 100조우':`보스 출현까지 ${fmt(Math.max(0,500-battles(s)))}전`;}
  ...
  function assessment(s,monsters,e=null){
    ...
    const party=monsters.map(m=>{const stats=window.forgeStats?.(m)||monsterStats(m);return {stats,skill:skillDef(m),hp:Math.min(stats.hp,Number(e?.battle_state?.partyHp?.[m.id]??m.field_hp??stats.hp))}});
  ...
  function hunt(){return `<div class="campaign-heading"><small>EXPANSION PLAN</small><h2>오늘의 박멸 영업</h2><p>산골 첫 영업부터 용사 왕국 인수까지.</p></div><div class="campaign-list">${S.sites.map(card).join('')}</div>`;}
  function nextStep(){
  ...
  function refresh(){if(!enabled())return;
    document.querySelectorAll('.campaign-card').forEach(el=>{const s=siteBy(el.dataset.campaignSite);if(!s)return;const e=expedition(s.id),p=s.progress||{},party=e?partyOf(e):[],alive=party.filter(m=>Number(e.battle_state?.partyHp?.[m.id]??1)>0).length,clears=Number(p.boss_victories||(p.boss_cleared?1:0));
      const text=(selector,value)=>{const node=el.querySelector(selector);if(node&&node.textContent!==value)node.textContent=value};
      el.classList.toggle('locked',!s.unlocked);text('.campaign-progress small',unlocked(s)?'출현 해금':`${fmt(Math.min(500,battles(s)))}/500전`);
      const bar=el.querySelector('.campaign-progress i'),width=(unlocked(s)?100:Math.min(100,battles(s)/5))+'%';if(bar.style.width!==width)bar.style.width=width;
      text('.campaign-live b',e?e.battle_state?.active&&e.battle_state?.boss?'보스 교전 중':`${e.phase||'탐색'} · 생존 ${alive}/${party.length}`:progressText(s));text('.campaign-live small',clears?`보스 ${fmt(clears)}회 처치 · ${date(p.boss_cleared_at)}`:clearReward(s));
      text('.campaign-card-actions [data-campaign-action="mission"]',e?'파티 확인':s.unlocked?'파티 편성':'해금 조건');text('.campaign-card-actions [data-action="collect-site"]','수거 '+fmt(sitePending(s.id)));
      el.querySelector('[data-action="collect-site"]').disabled=!siteHasCollectable(s.id);
      const cover=el.querySelector('.campaign-cover');cover.setAttribute('aria-label',s.name+' '+(s.unlocked?'파티 편성':'해금 조건'));text('.campaign-cover-copy>span',`권장 Lv.${s.chapter.level.join('–')}${s.unlocked?'':' · 이전 보스 처치 필요'}`);
      if(p.boss_cleared&&!cover.querySelector('.campaign-clear')){const tag=document.createElement('em');tag.className='campaign-clear';tag.textContent='인수 완료';cover.append(tag);}
    });
